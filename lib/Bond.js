...
Bond.prototype.dep = function dep ( bonds ) {
...
Bond.propagate = function propagate ( bond ) {
  ...
  for ( var i = 0; i < bond.deps.length; i += 1 ) {
    bond.deps[i].counter -= 1;

    Bond.execute(bond.deps[i]);
  }
...
if ( require.main === module ) {
  ...
  var f = new Bond().run(function ( callback ) { setTimeout(function ( ) { callback('error'); }, 5); });
  var pass = new Bond().dep(f).run(function ( callback ) { setTimeout(function ( ) { callback(); }, 5); }).timeout(15);

  pass.callback(function ( ) {
    console.log('execution did:     \033[3' + (pass.result.error ? '1mtimeout' : '2mcompleted') + '\033[30m');
  });
}
